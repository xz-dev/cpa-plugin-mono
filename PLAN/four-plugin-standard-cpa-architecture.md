...
Remains read-only. It displays the effective `/v1/models?client_version=1.0.0` catalog and cached/effective views. Because stock CPA exposes no plugin-host callback for the assembled client-facing model catalog and `/v1/models` is protected by the independent proxy `api-keys` middleware, Writer is the only catalog fetcher: a user/manual refresh calls Writer; Writer GETs authoritative config, selects exactly one existing root `api-keys` value by configured lowercase SHA-256 fingerprint, and makes a fixed direct-loopback GET to its own `/v1/models?client_version=1.0.0` with `Authorization: Bearer <normalized-key>`. Writer then re-GETs authoritative config before ingestion and requires the exact config version to remain unchanged; drift discards the fetched body, records `version_conflict`, and is never automatically replayed. Writer sends the size-limited catalog to `model-info`'s private `/ingest` route only after that check. `model-info` validates/parses/caches that catalog. It is never configured with, returns, logs, or retains the management credential or proxy API key; stock management dispatch transiently delivers Writer's Authorization header to the handler, which must ignore it. It must not call Writer commit or any CPA write endpoint. A direct external write after Writer's final recheck remains outside cooperative serialization and is an explicit residual boundary.
...
- one Writer worker goroutine executes all write, model-info, and reconcile jobs globally one at a time. It rechecks the blocker while selecting each queued job: queued config writes remain pending behind a newly established blocker, while later read-only model-info and reconcile jobs may pass them; an evidence-based clear wakes the worker and admits each retained write once;
...
- Writer stores one monotonic absolute next-run deadline per periodic operation. `Configure` atomically updates future settings without replacing the queue, commit mutex, `instance_id`, active job, or unchanged deadlines: a `sync_epoch`-only reconfigure preserves every deadline; changing one interval schedules only that operation from `configure_time + new_interval`; disabling removes only its deadline. Timer admission and deadline advancement occur under the same blocker lock: an expired write deadline advances only after its job is admitted or coalesced, remains expired while blocked, and enqueues exactly once after clear. A process restart creates fresh deadlines from successful startup reconciliation; there is no persisted timer catch-up. Each active job keeps an immutable run-scoped settings copy;
...
Writer's continuation request repeats the original `version` and `config_base64` and adds exactly the following fields. Runs intentionally have no selector-scope field: each invocation processes all selectors configured in the target planner, and ordinary operation-level coalescing remains unambiguous.
...
- `version_conflict` (Core class 409): precondition mismatch; include authoritative `version` only. Config-write planners recompute until their configured retry bound; model-info drift before ingest is terminal for that run and is not automatically replayed.
...
3. Writer invokes that plugin's authenticated `/plan` route with the base64 snapshot, opaque version, and private coordination token; one run processes every selector configured in that planner and there is no trigger-level selector scope;
